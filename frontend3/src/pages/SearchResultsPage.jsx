 ...
 * Premium Search Results Page with vertical bullet-point summarization.
 */
  ...
  return (
    <div style={styles.wrap}>
      <div style={styles.header} className="glass">
        ...
        <button style={styles.clearBtn} onClick={onClear}>Clear Results</button>
      </div>
...
function SourceCard({ src, idx, keywords }) {
  ...
  const [activeAction, setActiveAction] = useState(''); // track 'Summarize', 'Caption', etc.
  ...
  async function runAction(label, apiFn) {
    ...
    setAiResult('');
    try { 
      const r = await apiFn(clean); 
      const text = r.summary ?? r.caption ?? (r.status === 200 ? '✅ Done' : JSON.stringify(r));
      setAiResult(text); 
    }
  ...
  function formatSummaryAsBullets(text) {
    ...
    // Split by sentences if a single line paragraph
    if (lines.length === 1) {
      const sentences = lines[0].split(/(?<=\.)\s+/);
      lines = sentences.map(s => s.trim()).filter(s => s.length > 0);
    }

    // Strip raw bullet symbols
  ...
  return (
    <div style={styles.card} className="glass-panel">
      <div style={styles.cardHead}>
        ...
        <span className="search-result-subject" style={styles.cardSubj}>{src.subject ?? 'No Subject'}</span>
...
const styles = {
  ...
  aiRes:    { marginTop: 12, padding: '14px 16px', background: 'var(--ai-bubble-gradient)', border: '1px solid var(--b2)', borderRadius: 'var(--rad)', fontSize: 13.5, color: 'var(--t1)', borderLeft: '3px solid var(--maroon3)', boxShadow: '0 4px 12px var(--maroon-glow)' },
